...
model = dict(
    ...
    pts_middle_encoder=dict(
        type='PointPillarsScatter', in_channels=64, output_shape=(600, 80)
    ),
    pts_backbone=dict(
        type='SECOND',
        in_channels=64,
    ...
    pts_bbox_head=dict(
        ...
        bbox_coder=dict(
            ...
            post_center_range=[-10.0, -15.0, -4.0, 160.0, 15.0, 10.0], # todo
            score_threshold=0.0,
            code_size=8,
        ),
    ...
    train_cfg=dict( # 训练时和测试时传递的一些参数, 所有层都能获取到?
        pts=dict(
            ...
            min_radius=2,
            grid_size=[80, 600, 1],  # [x_len, y_len, 1] # 生成heatmap
            voxel_size=voxel_size,
            out_size_factor=out_size_factor,
