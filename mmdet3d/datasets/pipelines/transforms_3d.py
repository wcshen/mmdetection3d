...
@PIPELINES.register_module()
class RandomCenterGlobalRotScaleTrans(object):
    """Apply global rotation, scaling and translation to a 3D scene.
    ...
    def __init__(self,
        ...
        if not isinstance(rot_range, seq_types):
            ...
            rot_range = [-rot_range, rot_range]
        self.rot_range = rot_range
        self.farrest_dis = farrest_dis
        self.min_rot_frac = min_rot_frac
    ...
    def _trans_bbox_points_to_rot(self, input_dict, trans_factor):
        """Private function to translate bounding boxes and points.
    ...
    def _rot_bbox_points_to_angle(self, input_dict, rot_range):
        rotation = rot_range
        noise_rotation = np.random.uniform(rotation[0], rotation[1])
    ...
    def __call__(self, input_dict):
        ...
        rot_center = rot_centers[assign_rot_indedx] # torch.Tensor (3)
        rot_center_distance = torch.sqrt(rot_center[0]**2 + rot_center[1]**2).numpy()
        rot_frac = 1 - (1 - self.min_rot_frac) * rot_center_distance / self.farrest_dis
        # rot_center -> origin
        self._trans_bbox_points_to_rot(input_dict, -rot_center)
        rot_range = list(map(lambda x: x*rot_frac, self.rot_range))
        self._rot_bbox_points_to_angle(input_dict, rot_range=rot_range)
        # rot_center <- origin
        self._trans_bbox_points_to_rot(input_dict, rot_center)
